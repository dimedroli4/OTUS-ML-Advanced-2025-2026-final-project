...
## Почему такой хороший результат ?

1. Ярко выраженная мультисезонность

Сетевой трафик 5G имеет предсказуемые паттерны — люди спят ночью, работают днём, отдыхают в выходные. Модель легко выучила эти циклы.

2. Стабильность поведения пользователей

В краткосрочной перспективе (6 недель) нет резких изменений в поведении абонентов — что вчера было пиком, скорее всего, будет пиком и сегодня.

3. Высокая агрегация данных

Большое количество пространственных точек (лучей) и временных шагов создаёт богатый обучающий датасет. Модели не страдают от малого количества данных.

4. Схожесть поведения лучей

XGBoost обучается сразу на всех 2880 лучах (в цикле), но логи преобразуются индивидуально. При этом общие паттерны едины для всей сети.

5. Отсутствие длинных трендов

Прогнозировать тренды сложно. Их отсутствие упрощает задачу — достаточно выучить повторяющиеся паттерны.

6. Бинарная природа таргета (с порогом 0.5)

Бинарная классификация проще регрессии. Модель не обязана предсказывать точное число пользователей — достаточно определить состояние "активен/неактивен". Высокие Precision/Recall это подтверждают.

***Итог: почему модель так хороша?***

Фактор	                        Вклад в успех
Мультисезонность (24ч + 168ч)	⭐⭐⭐⭐⭐
...
Ключевая особенность данных, которая обеспечила успех:

***Сетевой трафик 5G на горизонте 5-6 недель — это почти идеальный стационарный временной ряд с чёткими суточными и недельными циклами и без трендов. Любая модель, способная выучить сезонность и краткосрочные зависимости, покажет хороший результат. Ансамбль просто довёл это до максимума.***
